...
Partition == {"p1", "p2"}
MaxPartitionLen == 5
MaxTime == MaxPartitionLen
\*MaxTime == MaxPartitionLen*2
...
Produce(event, partition) ==
    ...
    /\ produce_actions' = produce_actions \cup {[produce_event |-> event, produce_partition |-> partition, produce_time |-> time]}
    /\ time' = time + 1
...
Consume(consumer, partition, event) ==
    ...
    /\ consume_actions' = consume_actions \cup {[consume_consumer |-> consumer, 
        consume_partition |-> partition, 
        consume_event |-> event, 
        consume_time |-> time]}
    /\ time' = time + 1
